(function attachBackgroundAccountRunHistory(root, factory) {
  ...
  function createAccountRunHistoryHelpers(deps = {}) {
    ...
    function resolveRecordEmail(record = {}) {
      const identity = resolveRecordIdentity(record);
      return identity.email || String(record.registrationEmailState?.current || '').trim().toLowerCase();
    }

    function normalizeEmailPoolEntry(rawEntry = {}) {
      const parseEntryString = (value = '') => {
        const rawValue = String(value || '').trim();
        const separator = rawValue.includes('----') ? '----' : (rawValue.includes('---') ? '---' : '');
        const separatorIndex = separator ? rawValue.indexOf(separator) : -1;
        return {
          email: (separatorIndex >= 0 ? rawValue.slice(0, separatorIndex) : rawValue).trim().toLowerCase(),
          verificationUrl: normalizeRecordUrl(separatorIndex >= 0 ? rawValue.slice(separatorIndex + separator.length) : ''),
        };
      };

      if (!rawEntry || typeof rawEntry !== 'object' || Array.isArray(rawEntry)) {
        return parseEntryString(rawEntry);
      }

      const parsedEntry = parseEntryString(rawEntry.credential || rawEntry.email || '');
      return {
        email: String(parsedEntry.email || rawEntry.email || rawEntry.accountIdentifier || '').trim().toLowerCase(),
        verificationUrl: normalizeRecordUrl(rawEntry.verificationUrl || rawEntry.url || parsedEntry.verificationUrl || ''),
      };
    }

    function resolveRecordEmailVerificationUrl(record = {}, fallbackEmail = '') {
      const directUrl = normalizeRecordUrl(
      ...
      const email = String(fallbackEmail || resolveRecordEmail(record) || '').trim().toLowerCase();
      if (!email) {
        return '';
      }

      const pools = [
        Array.isArray(record.customEmailPoolEntries) ? record.customEmailPoolEntries : [],
        Array.isArray(record.customEmailPool) ? record.customEmailPool : String(record.customEmailPool || '').split(/\r?\n/),
      ];
      for (const entries of pools) {
        for (const rawEntry of entries) {
          const entry = normalizeEmailPoolEntry(rawEntry);
          if (entry.email === email && entry.verificationUrl) {
            return entry.verificationUrl;
          }
        }
      }
    ...
    function normalizeAccountRunHistoryRecord(record) {
      ...
      const totpMfaEnabledAt = String(record.totpMfaEnabledAt || '').trim();
      const gptPasswordSet = Boolean(record.gptPasswordSet);
      const gptPasswordSetAt = String(record.gptPasswordSetAt || '').trim();

      const normalizedRecordId = buildRecordId(accountIdentifier, accountIdentifierType);
      const rawRecordId = String(record.recordId || '').trim();
      const shouldUseRawRecordId = rawRecordId
        && !(accountIdentifierType === 'email' && email && rawRecordId.toLowerCase() !== normalizedRecordId);

      return {
        recordId: shouldUseRawRecordId ? rawRecordId : normalizedRecordId,
        flowId,
        runId,
        accountIdentifierType,
        accountIdentifier,
    ...
    function getAccountRunHistoryDedupeKeys(record = {}) {
      const keys = [];
    ...
    function keepLatestAccountRunHistoryRecords(records = []) {
      const seenKeys = new Set();
      const latestRecords = [];

      for (const record of records) {
        const keys = getAccountRunHistoryDedupeKeys(record);
    ...
    function normalizeAccountRunHistory(records) {
      ...
      const normalizedHistory = records
      ...
      return keepLatestAccountRunHistoryRecords(normalizedHistory);
    }

    async function getPersistedAccountRunHistory() {
      try {
        const stored = await chrome.storage.local.get(ACCOUNT_RUN_HISTORY_STORAGE_KEY);
        const rawHistory = Array.isArray(stored[ACCOUNT_RUN_HISTORY_STORAGE_KEY])
          ? stored[ACCOUNT_RUN_HISTORY_STORAGE_KEY]
          : [];
        const normalizedHistory = normalizeAccountRunHistory(rawHistory);
    ...
    function buildAccountRunHistoryRecord(state = {}, status = '', reason = '') {
      ...
      const totpMfaEnabledAt = String(state.totpMfaEnabledAt || '').trim();
      const gptPasswordSet = Boolean(state.gptPasswordSet);
      const gptPasswordSetAt = String(state.gptPasswordSetAt || '').trim();
