...
async function readChatGptSessionAccessToken(options = {}) {
  const forceRefresh = options.forceRefresh !== false;
  const sessionApiUrl = String(options.sessionApiUrl || CHATGPT_SESSION_API_URL).trim();
...
async function inspectPlusCheckoutState(options = {}) {
  ...
  if (options.includeSession || options.includeAccessToken) {
    const sessionState = await readChatGptSessionAccessToken(options);
