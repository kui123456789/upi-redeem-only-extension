(function attachPhoneAuthModule(root, factory) {
  ...
  function createPhoneAuthHelpers(deps = {}) {
    ...
    async function ensureCountrySelected(countryLabel, phoneNumber = '', timeout = 15000) {
      ...
      if (select) {
        while (Date.now() - start < timeout) {
          ...
          const targetOption = byLabel || byPhoneNumber;
          if (!targetOption) {
            await sleep(200);
            continue;
          }
      ...
      if (await trySelectCountryListboxOption(countryLabel, phoneNumber)) {
        const expectedDialCode = normalizePhoneDigits(resolveTargetDialCode(countryLabel, phoneNumber));
        const displayedDialCode = normalizePhoneDigits(getDisplayedDialCode());
        return {
          ok: true,
          dialCode: expectedDialCode || displayedDialCode || '',
          selectedLabel: getCountryButtonText(),
          displayedDialCode,
        };
      }

      const targetDialCode = normalizePhoneDigits(resolveTargetDialCode(countryLabel, phoneNumber));
      const displayedDialCode = normalizePhoneDigits(getDisplayedDialCode());
      return {
        ok: false,
        dialCode: lastSnapshot?.expectedDialCode || targetDialCode || '',
        displayedDialCode: lastSnapshot?.displayedDialCode || displayedDialCode || '',
        selectedLabel: getOptionLabel(lastSnapshot?.selectedOption),
      };
    }
    ...
    function getAddPhoneErrorText() {
      const form = getAddPhoneForm() || getAddPhoneRoot();
    ...
    async function submitPhoneNumber(payload = {}) {
      ...
      const displayedDialCode = normalizePhoneDigits(getDisplayedDialCode());
      const targetDialCode = normalizePhoneDigits(resolveTargetDialCode(countryLabel, payload.phoneNumber));
      const selectedDialCode = normalizePhoneDigits(countrySelection.dialCode || targetDialCode);
      const selectedMatchesTargetDialCode = !targetDialCode || selectedDialCode === targetDialCode;
      if (!selectedMatchesTargetDialCode) {
      ...
      if (selectedDialCode && displayedDialCode && displayedDialCode !== selectedDialCode) {
        throw new Error(`Failed to select "${countryLabel || 'target country'}" on the add-phone page. expected +${selectedDialCode} but displayed +${displayedDialCode}，当前显示为 ${getCountryButtonText() || displayedDialCode || '未知'}。`);
      }
