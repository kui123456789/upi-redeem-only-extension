(function attachStepDefinitions(root, factory) {
  ...
  const TOTP_MFA_STEP_DEFINITION = {
    ...
    sourceId: 'chatgpt',
    driverId: null,
    command: 'enable-totp-mfa',
  };
  const SET_GPT_PASSWORD_STEP_DEFINITION = {
    id: 6,
    order: 60,
    key: 'set-gpt-password',
    title: '设置 GPT 密码',
    sourceId: 'openai-auth',
    driverId: null,
    command: 'set-gpt-password',
  };
  ...
  function shouldStopUPIAfterRedeem(options = {}) {
    return (options?.upiRedeemContinueAfterRedeem ?? options?.pixRedeemContinueAfterRedeem) !== true;
  }

  function shouldEnableTotpMfaAfterProfile(options = {}) {
    return options?.totpMfaAfterProfileEnabled !== false;
  }
    ...
    openai: {
      getAllSteps() {
        ...
        for (const step of [
          ...
          ...PLUS_GPC_PHONE_STEP_DEFINITIONS,
          ...PLUS_GPC_PHONE_BOUND_EMAIL_RELOGIN_STEP_DEFINITIONS,
          ...PLUS_UPI_NO_TOTP_STEP_DEFINITIONS,
