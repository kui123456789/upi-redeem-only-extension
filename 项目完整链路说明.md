...
这是一个 Chrome 扩展，用于自动执行一整套 OpenAI / ChatGPT OAuth 注册、登录与 UPI 试用资格检测流程。

当前仓库是 UPI Redeem Only 专用版：PayPal、GoPay、GPC、IP 代理和手机号接码的专用加载入口与模块已经移除。文中如保留旧状态字段或兼容判断，主要用于历史配置迁移和防止旧状态误触发，不代表这些能力仍可用。
...
- UPI 专用版只展示 OAuth 账号接入与 UPI 试用资格检测主路径；PayPal、GoPay、GPC、IP 代理和手机号接码配置入口不再参与运行。
...
- 旧登录验证码页显示邮箱兼容字段 `step8VerificationTargetEmail`
...
1. 解析本轮应使用的注册方式：UPI 专用版固定使用邮箱注册
2. 打开或复用注册页
3. 后台打开或切回注册页后，会先等待标签页完成加载并额外稳定 3 秒，再连接内容脚本探测入口；这用于避开官网按钮已在 DOM 中但仍处于隐藏态的异步窗口期
4. 内容脚本识别到官网“免费注册 / Sign up / Register”入口后，会先等待 3 秒再点击；如果点击后仍停留在官网入口页、没有进入邮箱输入页，会再次等待 3 秒后重试，最多额外重试 5 次，且每次真正点击前都会检查 Stop 状态
...
7. UPI 专用版不再申请手机号或调用接码平台。
8. 以当前邮箱账号标识先写入一条“停止（流程尚未完成）”的记录占位
9. 等待身份提交后的真实落地页
10. 如果进入密码页，则继续执行 Step 3
11. 如果直接进入邮箱验证码页，则自动跳过 Step 3 并进入 Step 4
...
3. OpenAI 手机号验证流程会按当前 sidepanel 中保存的接码平台 `phoneSmsProvider` 选择 HeroSMS / 5sim / NexSMS / SMSBower / 托管短信接口，并使用对应平台的 API Key、国家/地区、价格上限或托管号码池申请号码；SMSBower 使用官方 `handler_api.php`，OpenAI 服务代码默认 `dr`，内置美国 `187`、马来西亚 `7`、加纳 `38`、泰国 `52` 等国家；托管短信接口的号码格式为 `2092905100----https://example.test/api/sms/recordText?key=replace-me`，10 位号码固定按美国 `+1` 本地号提交
4. 后台提交号码后，会按 `currentPhoneActivation.provider` 分发后续查码、完成、取消、ban、reuse：HeroSMS 走原 `getStatus / setStatus` 逻辑，5sim 走 `/v1/user/check|finish|cancel|ban|reuse`，NexSMS 走消息接口，SMSBower 走 `getStatus / setStatus`，托管短信接口只轮询配置的 URL 并追加 `t=Date.now()` 防缓存，完成/取消/封禁只更新本地使用记录
...
- HeroSMS / 5sim / NexSMS / SMSBower / 托管短信接口都归入 OpenAI 接码平台适配层；默认平台仍是 HeroSMS，5sim 产品码固定为 `openai`，operator 默认 `any`，SMSBower 服务代码默认 `dr`。SMSBower 和托管短信接口只处理 Auth/OAuth 后链 Step9 的后置手机号验证，不参与手机号注册、手机号登录验证码或 PayPal hosted checkout。该适配层暂不进入多注册 flow 的 core 通用服务，后续只有第二个真实 flow 也需要接码时再按实际复用点抽象。
...
- 5sim 的国家/地区使用字符串 slug（当前开放 `indonesia / thailand / vietnam`），HeroSMS 仍使用数字国家 ID（当前开放印度尼西亚/泰国/越南），SMSBower 使用官方国家 ID；美国 `+1` 返回 `+1XXXXXXXXXX` 或 `1XXXXXXXXXX` 时提交给 OpenAI 的号码会归一为本地 10 位 `XXXXXXXXXX`。
...
UPI 专用版固定使用 UPI 试用资格检测流程，不再创建 PayPal / GoPay checkout，也不再调用 GPC helper、手机号接码 helper 或卡密兑换提交。

可见步骤：

1. 第 1~5 步：打开 ChatGPT、提交注册邮箱、填写密码、获取注册邮箱验证码、填写资料。
2. 第 6 步：设置 GPT 登录密码。
3. 第 7 步：开通 TOTP 2FA、记录账号凭据备份，并检测账号是否有 UPI 试用资格；不执行卡密兑换。

以上复用步骤的日志标签、完成信号和错误信号都以结构化 `step` 为准，不再允许靠日志正文中的固定步骤号修正显示。

隐藏与跳过规则：

- PayPal、GoPay、GPC、旧 Plus Checkout 回跳、IP 代理和手机号接码节点不会进入 UPI 步骤定义。
- 如果旧状态或旧 UI 误触发这些节点，后台会直接提示该功能已从 UPI 专用版移除。
