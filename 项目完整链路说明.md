...
- 展示一个单独的“接码”开关、注册方式 `signupMethod` 与“接码平台”下拉；接码平台当前支持 HeroSMS / 5sim / NexSMS / SMSBower / 托管短信接口。普通模式下开启接码后可把注册方式切到手机号注册，并在 OAuth 登录链路命中手机号登录验证码页时继续复用同一手机号续跑短信验证；SMSBower 和托管短信接口只用于邮箱注册后 OAuth Step9 的 `add-phone / phone-verification` 后置手机号验证
