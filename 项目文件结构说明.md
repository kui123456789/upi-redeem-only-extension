...
- `manifest.json`：Chrome 扩展清单，声明权限、背景脚本、侧边栏、内容脚本与规则集；UPI 专用版不再声明 IP 代理相关权限。
...
- `content/plus-checkout.js`：ChatGPT 页面脚本，当前由 UPI 兑换链路复用来读取 ChatGPT 会话信息，不再驱动 PayPal / GoPay checkout。
...
- `sidepanel/sidepanel.css`：侧边栏样式文件；当前额外提供 Hotmail / 2925 共用的号池表单容器、操作按钮行、统一的收起态列表高度样式，以及 UPI-only 模式下旧非 UPI 配置区的隐藏规则。
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部保留贡献模式入口与更新轻提示；UPI 专用版只保留 OAuth 账号接入、邮箱注册、2FA 与 UPI 试用资格检测相关配置，PayPal、GoPay、GPC、IP 代理和手机号接码相关脚本入口已移除。
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、邮箱 provider 配置、账号池管理接线、贡献模式、账号记录面板与广播接收；UPI 专用版强制使用 UPI 步骤定义，旧 PayPal、GoPay、GPC、IP 代理和手机号接码配置区保持隐藏且不再加载对应 manager/provider 脚本。
...
- `tests/paypal-approve-detection.test.js`：旧 PayPal 授权执行器测试，UPI 专用版已不再加载 PayPal 流程。
